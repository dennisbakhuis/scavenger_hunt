...
def scavenger_admin():
    ...
    with overview_tab:
        ...
        if state.n_active_teams > 0:
            ...
            for team in state.teams.values():
                current_goal = team.goal_location_name if len(team.solved) < len(game.locations) else "All locations solved"
        ...
        st.markdown("**Enable beam-to-location for all teams.** All teams will see a beam to location checkbox when their UI refreshes. When this checkbox is enabled, you are automatically beamed to the current goal and can directly answer the question.")

        def change_beam_to_location_if_state_exists(state: State, beam_to_location: bool):
            """Change beam to location visibility."""
            if Path(STATE_FILE).exists() and state.button_beam_to_location_visible != beam_to_location:
        ...
        st.markdown("---")
        st.markdown("**Delete all team data.** This will delete all team data and reset the game. This should only be done if nobody is actually logged in as each client can restore their version of the state")
    ...
    with questions_tab:
        ...
        with col1:
            st.subheader(f"{st.session_state.index + 1} - {location.name}")
    ...
    with stats_tab:
        ...
        scores_collected = {
            team_name: {
                location.name: team_state.solved[location.name] if location.name in team_state.solved else 0
        ...
        no_beam_logs["distance"] = no_beam_logs.apply(
            lambda row: calculate_distance(row["latitude"], row["longitude"], row["prev_latitude"], row["prev_longitude"]),
            axis=1
        )
        ...
        summary_df = logs_sorted.groupby("team_name").apply(lambda group: pd.Series({
