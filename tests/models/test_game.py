...
def test_game_model_data():
    ...
    other_game = Game.from_yaml_file(file_path=GAME_FILE)

    assert game != other_game
    game._file_path = GAME_FILE
    assert game == other_game

    assert other_game._file_path == GAME_FILE
    assert other_game.file_path == GAME_FILE
